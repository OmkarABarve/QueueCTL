...
class SqliteStorage {
    constructor(filePath) {
        ...
        this.db = new better_sqlite3_1.default(this.filePath);
    }
    init() {
        this.db.exec(`
    ...
    `);
        ...
        const tx = this.db.transaction((nowISO) => {
            for (let i = 0; i < 5; i++) {
                const sel = selectStmt.get(nowISO, nowISO);
                if (!sel)
                    return null;
                const res = updateStmt.run({ now: nowISO, id: sel.id });
                if (res.changes === 1) {
                    return getStmt.get(sel.id);
                }
    ...
    list(state) {
        const rows = state
            ? this.db.prepare(`SELECT * FROM jobs WHERE state = ?`).all(state)
            : this.db.prepare(`SELECT * FROM jobs`).all();
        return rows.map((r) => Job_1.Job.fromJSON(this.rowToJSON(r)));
    }
    moveToDLQ(job) {
        const j = job.toJSON();
    ...
    listDLQ() {
        const rows = this.db.prepare(`SELECT payload FROM dlq ORDER BY dead_at ASC`).all();
        const out = [];
        for (const { payload } of rows) {
            try {
                const obj = JSON.parse(payload);
                out.push(Job_1.Job.fromJSON(obj));
            }
            catch {
    ...
    retryFromDLQ(id) {
        const tx = this.db.transaction(() => {
            const row = this.db.prepare(`SELECT payload FROM dlq WHERE id=?`).get(id);
            if (!row)
                return null;
            ...
            if (!parsed)
                return null;
            const j = Job_1.Job.fromJSON(parsed);
